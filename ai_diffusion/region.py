...
class Region(QObject, ObservableProperties):
    ...
    def link_target(layer: Layer):
        ...
        if parent := layer.parent_layer:
            if (
                not parent.is_root
                and parent.type is LayerType.group
                and not parent.name.startswith(("Layout Squirrel Regions", "LLM Layout Regions"))
            ):
                return parent
        return layer
...
class RootRegion(QObject, ObservableProperties):
    ...
    def add_region_for_layer(self, layer: Layer):
        """Create a region directly linked to an existing layer."""
        if self.is_linked(layer):
...
def process_regions(
    ...
    for layer, region in layer_regions:
        ...
        if _uses_full_strength_mask(layer, region):
            mask = Image.mask_threshold(mask)

        region_result = RegionInput(
            mask,
            layer_bounds,
            workflow.merge_prompt(region.positive, root.positive),
            conditioning_strength=region.conditioning_strength,
