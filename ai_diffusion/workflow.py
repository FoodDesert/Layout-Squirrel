...
use_etn_attention_mask = True
etn_attention_base_strength = 0.2
...
class Region:
    ...
    conditioning_feather: float = 0.25
    ...
    def from_input(i: RegionInput, index: int, language: str):
        ...
        return Region(
            ...
            i.conditioning_strength,
            i.conditioning_feather,
            control,
            i.loras,
...
def encode_prompt(
    ...
    if regions is None and len(cond.regions) > 0:
        return encode_prompt_with_region_masks(w, cond, clip, shape, image=image)
...
def encode_prompt_with_region_masks(
    w: ComfyWorkflow,
    cond: Conditioning,
    clip: Clip,
    shape: Extent | ImageReshape = no_reshape,
    ...
    for region in cond.regions:
        if region.is_background or not region.mask:
            continue
        region_positive = region.encode_prompt(w, clip, cond.style_prompt)
        mask = region.mask.load(w, shape)
        feather = _region_feather_px(region, shape)
        if feather > 0:
            mask = w.expand_mask(mask, grow=0, blur=feather, kernel="gaussian")
        region_positive = w.conditioning_set_mask(
            region_positive, mask, region.conditioning_strength
        )
        positive = w.conditioning_combine(positive, region_positive)

    return ConditioningOutput(positive, negative)


def _region_feather_px(region: Region, shape: Extent | ImageReshape):
    fraction = max(0.0, region.conditioning_feather)
    if fraction == 0.0:
        return 0

    feather = fraction * region.bounds.extent.shortest_side
    if region.mask.image is not None:
        source_extent = region.mask.image.extent
        target_extent = None
        if isinstance(shape, Extent):
            target_extent = shape
        elif shape.target_extent is not None:
            target_extent = shape.target_extent
        if target_extent is not None and source_extent.shortest_side > 0:
            feather *= target_extent.shortest_side / source_extent.shortest_side

    return max(0, round(feather))
...
def apply_attention_mask(
    ...
    if bottom_region.is_background:
        base_prompt = bottom_region.positive
        remaining = cond.regions[1:]
    else:
        base_prompt = cond.positive
        remaining = cond.regions

    extent = _attention_mask_extent(shape, remaining)
    if extent is None:
        regions = w.background_region(base_prompt.encode(w, clip, cond.style_prompt))
    else:
        base_mask = w.solid_mask(extent, etn_attention_base_strength)
        regions = w.define_region(None, base_mask, base_prompt.encode(w, clip, cond.style_prompt))
...
def _attention_mask_extent(shape: Extent | ImageReshape, regions: list[Region]):
    if isinstance(shape, Extent):
        return shape
    if shape.target_extent is not None:
        return shape.target_extent
    for region in regions:
        if region.mask.image is not None:
            return region.mask.image.extent
    return None
...
def scale_refine_and_decode(
    ...
    prompt = encode_prompt(w, cond, clip, regions, shape=extent.desired)
...
def generate(
    ...
    prompt = encode_prompt(w, cond, clip, regions, shape=extent.initial)
...
def inpaint(
    ...
    prompt = encode_prompt(w, cond_base, clip, regions, shape=extent.initial)
    ...
    if extent.refinement_scaling in [ScaleMode.upscale_small, ScaleMode.upscale_quality]:
        ...
        prompt_up = encode_prompt(w, cond_upscale, clip, regions, shape=shape)
...
def refine(
    ...
    prompt = encode_prompt(w, cond, clip, regions, in_image, extent.desired)
    model, prompt = apply_control(w, model, prompt, cond.all_control, extent.desired, vae, models)
    prompt = apply_reference_conditioning(
...
def refine_region(
    ...
    prompt = encode_prompt(w, cond, clip, regions, in_image, extent.initial)
