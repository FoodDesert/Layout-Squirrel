...
from .region import (
    ...
    get_layout_squirrel_latent_hint,
...
class DocumentModel(QObject, ObservableProperties):
    ...
    def _generate(self, queue_mode: QueueMode):
        ...
        _write_layout_squirrel_generation_attempt(
            "clicked",
            queue_mode=queue_mode.name,
            model=self,
        )
        ...
        _write_layout_squirrel_generation_attempt(
            ...
            queue_mode=queue_mode.name,
            model=self,
            workflow_kind=input.kind.name,
            seed=input.sampling.seed if input.sampling is not None else None,
    ...
    def _prepare_workflow(self, dryrun=False):
        ...
        region_layer = None
        ignored_layout_region_layer = False
        ...
        if mask is None:  # Check for region inpaint
            ...
            if _is_layout_squirrel_layer(region_layer):
                ignored_layout_region_layer = True
                region_layer = regions.layers.root
        ...
        if not dryrun:
            min_region_coverage = 0.001 if _has_layout_squirrel_regions(regions) else 0.02
            conditioning, job_regions = process_regions(
                regions, bounds, region_layer, min_coverage=min_region_coverage
            )
            conditioning.language = self.prompt_translation_language
            color_hint = None
            latent_hint = None
            if settings.llm_layout_color_hint_mode == "latent":
                latent_hint = get_layout_squirrel_latent_hint(regions, bounds, region_layer, seed)
            else:
                color_hint = get_layout_squirrel_color_hint(regions, bounds, region_layer, seed)
        else:
        ...
        if latent_hint is not None and input.images is not None and input.kind in (
            WorkflowKind.generate,
            WorkflowKind.refine,
        ):
            input.images.latent_hint_image = latent_hint
        if not dryrun:
            _write_layout_squirrel_debug(
                workflow_kind=workflow_kind,
                ui_strength=ui_strength,
                final_strength=strength,
                color_strength=settings.llm_layout_color_hint_strength,
                min_region_coverage=min_region_coverage,
                color_hint=color_hint,
                latent_hint=latent_hint,
                mask=mask,
                region_layer=region_layer,
                ignored_layout_region_layer=ignored_layout_region_layer,
                bounds=bounds,
    ...
    def _get_current_image(self, bounds: Bounds, exclude_internal=True):
        ...
        if exclude_internal:
            ...
            exclude.extend(l for l in self.layers.images if _is_layout_squirrel_layer(l))
    ...
    def handle_message(self, message: ClientMessage):
        ...
        if job.kind is JobKind.diffusion:
            _write_layout_squirrel_generation_attempt(
                ...
                image_count=len(message.images) if message.images is not None else None,
                error=message.error,
            )
...
def _write_layout_squirrel_debug(
    *,
    ...
    color_hint: Image | None,
    latent_hint: Image | None,
    mask: Mask | None,
    region_layer: Layer | None,
    ignored_layout_region_layer: bool,
    bounds: Bounds,
    conditioning: ConditioningInput,
    workflow_input: WorkflowInput,
):
    try:
        sampling = workflow_input.sampling
        data = {
            "workflow_kind_before_prepare": workflow_kind.name,
            "workflow_kind_after_prepare": workflow_input.kind.name,
            "ui_strength": ui_strength,
            "final_strength": final_strength,
            "min_region_coverage": min_region_coverage,
            "color_hint_mode": settings.llm_layout_color_hint_mode,
            "color_hint_seed": sampling.seed if sampling is not None else None,
            "color_hint_present": color_hint is not None,
            "color_hint_extent": list(color_hint.extent) if color_hint is not None else None,
            "latent_hint_present": latent_hint is not None,
            "latent_hint_extent": list(latent_hint.extent) if latent_hint is not None else None,
            "mask_present": mask is not None,
            "mask_bounds": list(mask.bounds) if mask is not None else None,
            "bounds": list(bounds),
            "region_layer_name": region_layer.name if region_layer is not None else None,
            "region_layer_is_root": region_layer.is_root if region_layer is not None else None,
            "ignored_layout_region_layer": ignored_layout_region_layer,
            "region_prompt_count": len(conditioning.regions),
            "region_prompts": [
                {
                    "positive": region.positive,
                    "bounds": list(region.bounds),
                    "conditioning_strength": getattr(region, "conditioning_strength", None),
                    "conditioning_feather": region.conditioning_feather,
                    "color_hint_strength": getattr(region, "color_hint_strength", None),
                }
                for region in conditioning.regions
            ],
            "color_hint_sha256": _image_sha256(color_hint) if color_hint is not None else None,
            "latent_hint_sha256": _image_sha256(latent_hint) if latent_hint is not None else None,
            "sampling_steps": sampling.total_steps if sampling is not None else None,
            "sampling_start_step": sampling.start_step if sampling is not None else None,
            "sampling_denoise_strength": sampling.denoise_strength if sampling is not None else None,
            "image_input_present": workflow_input.images.initial_image is not None
            if workflow_input.images is not None
            else False,
        }
        path = util.user_data_dir / "layout_squirrel_debug.json"
        path.write_text(json.dumps(data, indent=2), encoding="utf-8")
    except Exception as e:
        log.warning(f"Could not write Layout Squirrel debug data: {e}")


def _write_layout_squirrel_generation_attempt(stage: str, **extra):
    try:
        ...
        data = {
            ...
            "stage": stage,
            "color_hint_mode": settings.llm_layout_color_hint_mode,
            "layout_description": settings.llm_layout_description,
            "layout_tags": settings.llm_layout_tags,
        }
        if model is not None:
            data.update(
                {
                    "has_document": model.has_document,
                    "document_extent": list(model.document.extent) if model.has_document else None,
                    "active_layer": model.layers.active.name if model.has_document else None,
                    "strength": model.strength,
                    "is_editing": model.is_editing,
                    "region_only": model.region_only,
                    "region_count": len(model.active_regions) if model.has_document else None,
                    "executing_job_count": model.jobs.count(JobState.executing)
                    if model.has_document
                    else None,
                }
            )
        data.update(extra)
        path = util.user_data_dir / "layout_squirrel_generation_attempts.jsonl"
        with path.open("a", encoding="utf-8") as file:
            file.write(json.dumps(data) + "\n")
    except Exception as e:
        log.warning(f"Could not write Layout Squirrel generation attempt: {e}")


def _image_sha256(image: Image):
    return hashlib.sha256(bytes(image.to_bytes())).hexdigest()


def _is_layout_squirrel_layer(layer: Layer):
    while layer is not None and not layer.is_root:
...
def _has_layout_squirrel_regions(regions: RootRegion):
    return any(_is_layout_squirrel_layer(layer) for layer in regions.layers.all)
