...
class Region:
    ...
    color_hint_strength: float = 0.45
...
def generate(
    ...
    if latent_hint is not None:
        hint_image, hint_mask = w.load_image_and_mask(Image.scale(latent_hint, extent.initial))
        latent = w.layout_squirrel_latent_color_hint(latent, hint_image, hint_mask, 1.0)
    prompt = encode_prompt(w, cond, clip, regions)
...
def create(i: WorkflowInput, models: ClientModels, comfy_mode=ComfyRunMode.server) -> ComfyWorkflow:
    ...
    if i.kind is WorkflowKind.generate:
        return generate(
            ...
            ensure(i.images).latent_hint_image,
        )
