...
class Model(QObject, ObservableProperties):
    ...
    def apply_result(
        ...
        use_region_apply = (
            len(params.regions) > 0
            and region_behavior is not ApplyRegionBehavior.none
            and not _uses_llm_layout_regions(self.layers, params.regions)
        )
        if not use_region_apply:
...
def _uses_llm_layout_regions(layers, regions: list[JobRegion]):
    for region in regions:
        layer = layers.find(QUuid(region.layer_id))
