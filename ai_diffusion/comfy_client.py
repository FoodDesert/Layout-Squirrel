...
class JobInfo:
    ...
    @staticmethod
    def create_raw(workflow: dict):
        return JobInfo(str(uuid.uuid4()), None, raw_workflow=workflow, node_count=len(workflow))
...
class ComfyClient(Client):
    ...
    async def enqueue_raw_workflow(self, workflow: dict, front: bool = False):
        job = JobInfo.create_raw(workflow)
        self._queue.put(job, front=front)
        return job.id
    ...
    async def _run_job(self, job: JobInfo):
        if job.raw_workflow is not None:
            workflow = ComfyWorkflow.from_dict(job.raw_workflow)
        ...
        comfy_api_key = _read_comfy_org_api_key()
        if comfy_api_key:
            data["extra_data"] = {"api_key_comfy_org": comfy_api_key}
        ...
        async for msg in websocket:
            if isinstance(msg, bytes):
                ...
                if msg["type"] == "executing" and msg["data"]["node"] is None:
                    ...
                    is_raw_workflow = bool(self._active_job and self._active_job.raw_workflow)
                    if self._clear_job(job_id):
                        if len(images) == 0:
                            # It may happen if the entire execution is cached and no images are sent.
                            images = last_images
                        if len(images) == 0 and is_raw_workflow:
                            await self._report(
...
def _read_comfy_org_api_key():
    if settings.comfy_api_key.strip():
        return settings.comfy_api_key.strip()

    key = os.environ.get("KRITA_AI_DIFFUSION_COMFY_API_KEY", "").strip()
    if key:
        return key

    key_file = util.user_data_dir / "comfy_api_key.txt"
    try:
        return key_file.read_text(encoding="utf-8").strip()
    except FileNotFoundError:
        return ""
    except Exception as e:
        log.warning(f"Could not read Comfy API key file {key_file}: {e!s}")
        return ""
